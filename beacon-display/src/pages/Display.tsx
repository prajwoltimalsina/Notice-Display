...
//             variant="secondary"
//             size="icon"
//             className="backdrop-blur-sm bg-white/20 hover:bg-white/40 border-0 text-white"
//           >
...
//           <NoticeSlideshow notices={notices} intervalMs={15000} />
//         </div>

//         {/* Right Sidebar - Fixed width */}
//         <div className="w-48 md:w-56 lg:w-64 flex-shrink-0 overflow-hidden shadow-2xl">
//           <DisplaySidebar />
//         </div>
//       </div>
...
export default function Display() {
  const { notices, isLoading, refetch } = useNotices(true);
  const { getSetting, isLoading: settingsLoading } = useDisplaySettings();
  const { user, isAdmin } = useMongoAuth();

  // ── Auto-reload every 15s — safety net for missed socket events ──
  useEffect(() => {
    const poll = setInterval(refetch, 15000);
    return () => clearInterval(poll);
  }, [refetch]);

  // ── Re-fetch instantly when tab/screen becomes visible again ──
  useEffect(() => {
    const onVisible = () => {
